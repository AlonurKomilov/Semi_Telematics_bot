...
class TestAccountSettingsRoundtrip:
    ...
    async def test_storage_backend_keys_roundtrip(self, seeded):
        ...
        await d["db"].set_account_setting(a, STORAGE_BACKEND_KEY, "gdrive")
        await d["db"].set_account_setting(a, STORAGE_GDRIVE_REFRESH_TOKEN, "encrypted_blob_xyz")
        await d["db"].set_account_setting(a, STORAGE_GDRIVE_ROOT_FOLDER_ID, "1AbCdEfGh")
        await d["db"].set_account_setting(a, STORAGE_GDRIVE_USER_EMAIL, "user@example.com")

        assert await d["db"].get_account_setting(a, STORAGE_BACKEND_KEY) == "gdrive"
        assert await d["db"].get_account_setting(a, STORAGE_GDRIVE_REFRESH_TOKEN) == "encrypted_blob_xyz"
        assert await d["db"].get_account_setting(a, STORAGE_GDRIVE_ROOT_FOLDER_ID) == "1AbCdEfGh"
        assert await d["db"].get_account_setting(a, STORAGE_GDRIVE_USER_EMAIL) == "user@example.com"
    ...
    async def test_disconnect_clears_token(self, seeded):
        ...
        await d["db"].set_account_setting(a, STORAGE_GDRIVE_REFRESH_TOKEN, "tok")
        await d["db"].set_account_setting(a, STORAGE_BACKEND_KEY, "gdrive")

        # Disconnect flow
        await d["db"].set_account_setting(a, STORAGE_GDRIVE_REFRESH_TOKEN, "")
        await d["db"].set_account_setting(a, STORAGE_BACKEND_KEY, "disk")

        assert await d["db"].get_account_setting(a, STORAGE_GDRIVE_REFRESH_TOKEN) == ""
        assert await d["db"].get_account_setting(a, STORAGE_BACKEND_KEY) == "disk"
