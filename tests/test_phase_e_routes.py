...
async def phase_e_app(pg_db, monkeypatch):
    ...
    monkeypatch.setattr(
        "capabilities.telemetry.warehouse_reader.get_vehicle_telemetry_hourly",
        _fake_timeline,
    )
    ...
    monkeypatch.setattr(
        "capabilities.telemetry.warehouse_reader._enabled", lambda: True,
    )
...
class TestVehicleTimelineRoute:
    ...
    async def test_empty_points_when_flag_off(self, phase_e_app, monkeypatch):
        monkeypatch.setattr(
            "capabilities.telemetry.warehouse_reader._enabled", lambda: False,
        )
