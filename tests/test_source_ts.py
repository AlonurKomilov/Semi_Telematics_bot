"""source_ts propagated through the warehouse, never minted.
...
This file is deliberately in the ROOT suite, not in a package: it checks
that the value survives snapshot → hourly → daily, that no warehouse
table shadows one in public, and that every contract table carries the
column — reaching across features/vehicles/warehouse,
capabilities/integrations, capabilities/data_lifecycle and the physical
schema at once.  It belongs to no single package.

The pure arithmetic and the flooring rule live with the package that
defines them: capabilities/data_lifecycle/tests/test_staleness_contract.py.
"""
