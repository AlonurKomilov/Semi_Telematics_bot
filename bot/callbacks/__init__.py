...
from bot.helpers import _show, _render_audit_log, _safe_error
from bot.auth import _get_user, _group_chat_guard
from bot.i18n import t
from bot.callback_router import CallbackRouter

# ── Import command functions for simple route delegation ─────────

from bot.registration import cmd_start
...
from bot.management import cmd_account, cmd_users
from bot.admin import cmd_admin, cmd_accounts, cmd_sys_ai_stats, cmd_sys_server
from bot.scorecards import cmd_scorecards, cmd_scorecards_pdf, cmd_scorecards_csv
from bot.fuel_costs import cmd_fuelcost, cmd_fuelcost_add, cmd_fuelcost_summary
...
from bot.routes import cmd_route, cmd_route_go
from bot.geofences import cmd_geofences
from bot.events import cmd_events, cmd_events_text, cmd_events_csv
from bot.work_schedules import (
    cmd_work_schedules, cmd_wsched_add, cmd_wsched_view,
    cmd_wsched_rename, cmd_wsched_hours, cmd_wsched_start_hour,
    cmd_wsched_end_hour, cmd_wsched_changerole, cmd_wsched_role,
    cmd_wsched_delete,
)
from bot.ai import (
...
from bot.callbacks import navigation, settings, company, users, groups, parking
from bot.callbacks.text_handlers import handle_text  # re-exported
...
_router.exact("cmd_work_schedules", cmd_work_schedules)
_router.exact("wsched_add", cmd_wsched_add)
...
async def _wsched_start(u, c):
    ...
    await cmd_wsched_start_hour(u, c, schedule_id=sid, hour=hour)
...
async def _snooze_pick(u, c):
    try:
        ack_id = int(u.callback_query.data.replace("snooze_pick_", ""))
    except (ValueError, IndexError):
        await u.callback_query.answer("Invalid alert", show_alert=True)
        return
    await handle_snooze_pick(u, c, ack_id=ack_id)

async def _snooze_set(u, c):
    try:
        parts = u.callback_query.data.replace("snooze_set_", "").rsplit("_", 1)
        ack_id = int(parts[0])
        minutes = int(parts[1])
    except (ValueError, IndexError):
        await u.callback_query.answer("Invalid snooze", show_alert=True)
        return
    await handle_alert_snooze(u, c, ack_id=ack_id, minutes=minutes)

async def _snooze_back(u, c):
    try:
        ack_id = int(u.callback_query.data.replace("snooze_back_", ""))
    except (ValueError, IndexError):
        await u.callback_query.answer("Invalid alert", show_alert=True)
        return
    from bot.alerts import _restore_alert_keyboard
    await _restore_alert_keyboard(u, c, ack_id=ack_id)

async def _snooze_alert(u, c):
    try:
        ack_id = int(u.callback_query.data.replace("snooze_alert_", ""))
    except (ValueError, IndexError):
        await u.callback_query.answer("Invalid alert", show_alert=True)
        return
    await handle_alert_snooze(u, c, ack_id=ack_id)

_router.prefix("ack_alert_", _ack_alert)
_router.prefix("snooze_pick_", _snooze_pick)
_router.prefix("snooze_set_", _snooze_set)
_router.prefix("snooze_back_", _snooze_back)
_router.prefix("snooze_alert_", _snooze_alert)
...
async def _back_alert(u, c):
    try:
        ...
        await u.callback_query.answer("Invalid alert", show_alert=True)
        return
    from bot.alerts import handle_back_to_alert
