...
from database.knowledge_db import KB_CATEGORIES
from bot.config import logger, get_platform_db
...
def _can_view_article(user, article: dict) -> bool:
    """Check if user can see this article (for single-article views)."""
    vis = article.get("visibility", "private")
    target = article.get("target_role", "all")
    if article.get("created_by") == user.telegram_id:
        return True
    if vis == "private":
        return article.get("account_id") == user.account_id
    # public — must be approved for cross-account visibility
    if not article.get("approved"):
        return False
    return target == "all" or target == user.role.value
