...
**Money is NOT here.** Plans, offers, discounts, invoices, rollouts and
the billing-mode probe are the operator's half of the billing domain and
live with it, at
[capabilities/platform/billing/operator/router.py] — same ``/system``
prefix, same gate, so no URL changed when they moved out.  They were
here for most of a year, which is how this file reached six domains and
3,100 lines; ``tests/test_layer_boundaries.py`` now refuses their
return.  Comp grant / renew / revoke / history live under
``/billing/comp/*`` on the customer-side router, behind the same
``SYSTEM_OWNER_IDS`` gate.
"""
...
#: The personas a group may be scoped to.
...
async def force_refresh_vehicles(
    ...
    Wraps ``capabilities.integrations.shared.vehicle_state.ingest_vehicle_state`` —
