...
from interfaces.api.deps import require_permission, require_permission_any, get_tenant_db, get_user_truck_nums, get_user_company_codes, validate_company_access, filter_by_allowed_companies
...
from capabilities.events.severity import EVENT_SEVERITY, classify_event_severity as _classify_severity
...
async def scorecards(
    ...
    truck_filter: list[str] | None = await get_user_truck_nums(user) if own_perm else None
    drivers = await _svc_driver_efficiency(user["account_id"], days=days, company=company, truck_nums=truck_filter)
