...
const buttonVariants = cva(
  "group/button inline-flex shrink-0 items-center justify-center rounded-lg border border-transparent bg-clip-padding text-sm font-medium whitespace-nowrap transition-control outline-none select-none focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/50 [&:not([aria-haspopup])]:active:translate-y-px disabled:pointer-events-none disabled:opacity-50 aria-invalid:border-destructive aria-invalid:ring-3 aria-invalid:ring-destructive/20 dark:aria-invalid:border-destructive/50 dark:aria-invalid:ring-destructive/40 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
  {
    variants: {
      variant: {
      ...
      // `min-h-tap` (and `min-w-tap` on the square ones) on every step:
      // a 24px pointer-target floor that does NOT ride the Size axes, so
      // the box stops shrinking there while everything around it keeps
      // going. At the default multiplier this changes nothing — each
      // min-height equals the height already declared beside it — which
...
// ``forwardRef`` so callers (e.g. DataGrid's Manage-columns popover
// anchor) can attach a ref to the underlying button.  Without this,
// ``<Button ref={...}>`` would silently drop the ref on React 18
// (the primitive itself is forward-ref'd, but the wrapper wasn't).
