...
const COLOUR_LITERAL_ALLOWED: { file: string; why: string }[] = [
  ...
  { file: 'mods/store/items/theme/index.ts',
...
describe('UI chrome', () => {
  ...
  it('never adds a raw form control below the tap floor', () => {
    ...
    const BASELINE = { input: 235, textarea: 21, select: 16 };
  ...
  it('every accent the picker offers has a rule in both modes', () => {
    ...
    const registry = readFileSync(join(SRC, 'preferences/registry.ts'), 'utf8');
    const packs = readFileSync(join(SRC, 'mods/store/items/theme/index.ts'), 'utf8');
    ...
    // From the THEME_PACKS array ONLY. This used to read catalogue.ts,
    // which also declares MODS — entries with ids too, and a mod is a
    ...
    // were added. The packs have their own file now, so the collision is
    ...
    expect(packs, 'mods/store/items/theme/index.ts declares MODS — accents and mods share a file again')
      .not.toMatch(/\bMODS\s*[:=]/);
    ...
    expect(accents.length, 'no packs found in mods/store/items/theme/index.ts').toBeGreaterThan(0);
    ...
    // a token — see `accentSeed` in mods/store/items/theme.
  });
  ...
  it('the public brand tint sets every accent-derived token', () => {
    // Assembled: the accent blocks live in `mods/store/items/theme/*.css` now,
