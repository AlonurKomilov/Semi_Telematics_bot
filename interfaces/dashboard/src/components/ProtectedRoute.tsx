...
import { CardSkeleton, ErrorState } from './shell';
...
export default function ProtectedRoute({ permission, children }: ProtectedRouteProps) {
  ...
  const { viewHasAny, viewPermsReady, viewPermsError, refreshPermissions } = useRoleView();
  const { user } = useAuth();
  ...
  if (viewPermsError) return <ErrorState message="Could not load permissions for this preview. Retry or return to My dashboard."
