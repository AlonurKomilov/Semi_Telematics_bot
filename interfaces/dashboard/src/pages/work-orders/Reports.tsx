...
import { TrendingUp, TrendingDown, Minus, Download, Receipt, DollarSign, Wallet, Users } from 'lucide-react';
import { apiJSON, apiFetch } from '../../api/client';
import {
  PageHeader, EmptyState, ErrorState, CardSkeleton,
...
export default function Reports() {
  ...
  const summaryQuery = useQuery<SummaryResponse>({
    ...
    queryFn: () => apiJSON<SummaryResponse>(`/work-orders/reports/summary?days=${days}`),
  });
  const perVehicle = useQuery<ReportResponse>({
    queryKey: ['wo-reports', 'per-vehicle', days],
    queryFn: () => apiJSON<ReportResponse>(`/work-orders/reports/per-vehicle?days=${days}`),
  });
  const perType = useQuery<ReportResponse>({
    queryKey: ['wo-reports', 'per-task-type', days],
    queryFn: () => apiJSON<ReportResponse>(`/work-orders/reports/per-task-type?days=${days}`),
  });
  const perVendor = useQuery<ReportResponse>({
    queryKey: ['wo-reports', 'per-vendor', days],
    queryFn: () => apiJSON<ReportResponse>(`/work-orders/reports/per-vendor?days=${days}`),
  });
  const monthly = useQuery<MonthlyResponse>({
    queryKey: ['wo-reports', 'monthly-trend', days],
    queryFn: () => apiJSON<MonthlyResponse>(`/work-orders/reports/monthly-trend?days=${days}`),
  });
  ...
  if (firstError) {
    return (
      <div>
        <PageHeader icon={TrendingUp} title={t('cost_reports.title')} description={t('cost_reports.description')} />
  ...
  return (
    <div>
      <PageHeader
        icon={TrendingUp}
        title={t('cost_reports.title')}
        description={t('cost_reports.description')}
        actions={
