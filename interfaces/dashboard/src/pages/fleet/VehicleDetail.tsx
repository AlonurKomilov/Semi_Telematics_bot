...
export default function VehicleDetail() {
  ...
  if (!vehicle) return <p className="text-gray-500">Loading...</p>;

  const v = vehicle;
  const loc = v.location || {};
  const fuel = v.fuelPercent ?? v.fuel_percent;
  const faultList: Fault[] = faults?.faults || v.faults || v.fault_codes || [];
  const h: HealthData = health?.health || {};
  const healthAlerts: string[] = health?.alerts || [];

  return (
    <div>
      <Link to="/fleet/vehicles" className="text-blue-400 hover:underline text-sm mb-4 inline-block">
        ← Back to vehicles
      </Link>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-5 space-y-3">
          ...
          {v.defPercent != null && <Row label="DEF" value={`${Math.round(v.defPercent)}%`} />}
        </div>
        ...
        {health && (
          <div className="bg-gray-900 border border-gray-800 rounded-xl p-5 space-y-3">
            ...
            {healthAlerts.length > 0 && (
              <div className="mt-2">
                <p className="text-sm text-gray-400 mb-1">Health Alerts:</p>
                {healthAlerts.map((a, i) => (
                  <span key={i} className="inline-block bg-red-500/20 text-red-400 text-xs rounded px-2 py-0.5 mr-1 mb-1">
                    {a}
                  </span>
                ))}
              </div>
                ...
                <div key={i} className="bg-gray-800 rounded-lg p-3 text-sm">
                  <div className="flex items-center gap-2">
                    <span className="font-mono text-orange-400">
                      {f.j1939?.spnDescription || f.spnDescription || f.code || 'DTC'}
                    </span>
                    {f.j1939?.fmiDescription && (
                      <span className="text-xs text-gray-500">FMI: {f.j1939.fmiDescription}</span>
                    )}
                  </div>
                  {f.description && <p className="text-gray-400 mt-1">{f.description}</p>}
                  {(f.j1939?.txId != null || f.occurrences != null) && (
                    <div className="flex gap-3 mt-1 text-xs text-gray-500">
                      {f.occurrences != null && <span>× {f.occurrences} occurrence{f.occurrences !== 1 ? 's' : ''}</span>}
                      {f.j1939?.sourceAddressName && <span>Source: {f.j1939.sourceAddressName}</span>}
                    </div>
                  )}
                </div>
              ))}
