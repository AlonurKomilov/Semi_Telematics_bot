...
describe('a look is on only while it adds up', () => {
  ...
  it('recognises its own axes', () => {
    expect(activeModId(cab.accent, cab.radius!, cab.size!)).toBe('cab');
  });
  ...
  it('goes quiet the moment any axis is tweaked', () => {
    ...
    expect(activeModId(cab.accent, 'sharp', cab.size!)).toBe('');
    expect(activeModId(cab.accent, cab.radius!, 1)).toBe('');
    expect(activeModId('blue', cab.radius!, cab.size!)).not.toBe('cab');
  });
  ...
  it('survives a float round-trip', () => {
    ...
    expect(activeModId(cab.accent, cab.radius!, cab.size! + 1e-9)).toBe('cab');
    expect(activeModId(cab.accent, cab.radius!, cab.size! + 0.01)).toBe('');
  });

  it('answers empty when the axes match nothing', () => {
    expect(activeModId('blue', 'rounded', 1)).toBe('');
  });
});
