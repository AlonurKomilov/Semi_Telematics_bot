 ...
 *   • permission  (e.g. ``can_view_fuel`` for low-fuel trucks), AND
 ...
 *     Safety has ``can_view_fuel`` via role permissions.
 *
...
export default function OverviewAlertStrip({ stats, has, kpiPriority }: OverviewSectionProps) {
  ...
  if ((stats.low_fuel ?? 0) > 0 && relevant.has('lowFuel') && has('can_view_fuel')) {
    alertItems.push({
