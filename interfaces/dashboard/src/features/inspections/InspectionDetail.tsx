...
export function InspectionDetail({ inspectionId, onClose, onReviewed, onResent }: Props) {
  ...
  return (
    ...
    {/* Was a hand-rolled backdrop + panel: no focus trap, no Escape, no
        ``aria-modal``, no background scroll lock.  The header keeps its
        ``sticky top-0`` — it sits INSIDE the scroll region, which is what
        that positioning is for. */}
    ...
    {/* Portaled to body by base-ui, so tree placement is cosmetic — the
        "work order already open" confirm from the bridge.  Kept OUTSIDE
        <Sheet>, whose children must be its own parts. */}
    {bridgeDialog}
    </>
  );
}
