...
const FROZEN_KEYS: readonly string[] = [
  ...
  'mods.sound.acts',
  'mods.sound.acts.controls',
  'mods.sound.acts.places',
  'mods.sound.acts.selection',
  'mods.sound.acts.pack',
  'mods.sound.snoozeUntil',
...
describe('the axis set itself', () => {
  it('pins every key, because deleting one is as much a decision as adding one', () => {
    ...
    expect(Object.keys(MOD_DEFAULT).sort()).toEqual([
      ...
      'material', 'mode', 'motion', 'radius', 'shader', 'wallpaper', 'wallpaperDesk',
      'wallpaperLive', 'wallpaperPage',
    ]);
  });
});
