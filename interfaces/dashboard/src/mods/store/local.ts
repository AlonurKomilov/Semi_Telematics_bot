 ...
 * a CSS item reaches the page through the `@import` chain in
 * `index.css`, an icon set through `import()`, so every item the store
 * lists is already here. Nothing is fetched, and this file does not
 * pretend otherwise — it is the READ MODEL of the store, not a
 * downloads folder. When an item is one day delivered instead of built
 ...
 * an item calls it `id`, and a helper that guesses would silently offer
...
const has = (_row: ItemRow): boolean => true;
