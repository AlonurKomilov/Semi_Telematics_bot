...
describe('the Inventory panel holds its shape', () => {
  ...
  it('refuses to shrink the two fixed regions', () => {
    ...
    const card = src.slice(src.indexOf('className="sheet"'));
  ...
  it('keeps the card capped and scrolling rather than unbounded', () => {
    ...
    const card = src.slice(src.indexOf('className="sheet"'));
    // The ceiling is the PERSON's number now, not a constant — 70 is
    // only where the splitter starts.  What must stay true is that a
    // ceiling exists at all and the card scrolls inside it.
    expect(card).toContain('maxHeight: `${cardPct}%`');
    ...
    // …and the number can never leave the range that keeps BOTH regions
    // on screen: an unclamped drag is an unbounded card by another route.
    expect(src).toContain('fallback={70}');
  });
});
