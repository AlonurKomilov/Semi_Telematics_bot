...
- Surfaces audited: 2 | Not yet audited: Location card, Health card, vehicles list rows (phase 3 — planned)
...
1. Phase 3: wire Location card, Health card (per-stat times), and vehicles-list rows (`captured_at` as row-level "as of") — the list is where dispatchers make routing decisions. `Impact: high · Effort: S`
