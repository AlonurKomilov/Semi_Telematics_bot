...
import ai
from ai import _chat_histories
from ai.registry import DEFAULT_LOCATION
from permissions import is_management_role
from api.deps import require_permission, get_current_user, get_platform_db, get_tenant_db
...
def _parse_suggestions(text: str) -> tuple[str, list[str]]:
    """Extract '>> ...' suggestion lines from AI response."""
    lines = text.split("\n")
    clean, suggestions = [], []
    for line in lines:
        ...
        if m:
            ...
            clean.append(line)
    return "\n".join(clean).strip(), suggestions


async def _log_usage(account_id: int, user_id: int, action: str):
    """Log AI token usage from the last call."""
    usage = ai.get_last_usage()
    if not usage:
        return
    model_name = ai.get_account_model_name(account_id) or ai.get_current_model_name()
    try:
        from bot.config import get_platform_db as _gpdb
        pdb = await _gpdb()
...
async def _get_user_info(user: dict, platform_db) -> tuple[dict | None, list[str] | None, str]:
    ...
    user_context = {
        "name": user_obj.display_name or "",
        "role": role_val,
        "department": getattr(user_obj, "department", "general") or "general",
        "truck_num": truck_nums[0] if truck_nums else "",
        "truck_nums": truck_nums or [],
        "timezone": getattr(user_obj, "timezone", "America/New_York") or "America/New_York",
    }
    truck_filter = truck_nums if role_val == "driver" and truck_nums else None
...
async def clear_history(
    ...
    uid = int(user["sub"])
    ai.clear_history(uid)  # sync function
    return {"ok": True}
