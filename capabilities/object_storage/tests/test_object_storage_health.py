...
from adapters.storage.object_storage import (
    OBJECT_STORAGE_BACKEND_KEY, OBJECT_STORAGE_GDRIVE_REFRESH_TOKEN, OBJECT_STORAGE_GDRIVE_USER_EMAIL,
)
from adapters.storage.object_storage_sync import (
    STATE_LOCAL, STATE_REMOTE, STATE_STUCK, STATE_SYNCING,
)
from capabilities.object_storage.router import get_storage_health
...
class TestStorageHealth:
    ...
    async def test_queue_pending_vs_stuck_split(self, db):
        ...
        for i in range(3):
            ...
            qids.append(qid)
        from adapters.storage.object_storage_sync import ERR_TOKEN_EXPIRED
    ...
    async def test_drive_connected_state(self, db):
        ...
        # exchanges the token.
        await db.set_account_setting(acct.id, OBJECT_STORAGE_BACKEND_KEY, "gdrive")
        await db.set_account_setting(acct.id, OBJECT_STORAGE_GDRIVE_REFRESH_TOKEN, "encrypted-blob")
        await db.set_account_setting(acct.id, OBJECT_STORAGE_GDRIVE_USER_EMAIL, "adam@example.com")
        body = await get_storage_health(
            user={"account_id": acct.id, "sub": 1}, tenant_db=db,
        )
    ...
    async def test_email_hidden_when_disconnected(self, db):
        ...
        acct = await db.create_account("Acme")
        await db.set_account_setting(acct.id, OBJECT_STORAGE_GDRIVE_USER_EMAIL, "old@example.com")
        body = await get_storage_health(
            user={"account_id": acct.id, "sub": 1}, tenant_db=db,
        )
        assert body["drive"] == {"connected": False, "email": None}
