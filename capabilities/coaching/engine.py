    ...
    for did, card in cards_by_driver.items():
        ...
        for rule in rules:
            ...
            if rule.kind == KIND_SCORE_THRESHOLD:
                ...
                count = (counts_by_driver.get(did) or {}).get(key_et, 0)
                entry = evaluate_incident_rule(rule, count)
            if entry is None:
                continue

            # Idempotency: skip if pending exists for (driver, topic)
            already = await tenant.has_pending_coaching_assignment(
                account_id, did, entry.topic_key, rule.period_days,
            )
            if already:
                continue
