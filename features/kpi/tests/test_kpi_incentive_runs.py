...
class TestLifecycle:
    ...
    async def test_inactive_days_lower_the_bar_and_recompute(self, seeded):
        async with await _client(seeded["app"]) as c:
            ...
            # 14 inactive of 28: 14 active -> target 16,000; 17,000 >=
            # it, rpm 2.10 -> the requires_target 1.5% tier now applies.
            # (The typed number REPLACES the 10 auto "no loads" marks.)
            r = await c.patch(
                f"/api/kpi/dispatch/runs/{run['id']}/rows/{row['id']}",
                json={"inactive_days": 14, "inactive_reason": "repair"},
                headers=_h(seeded["owner"]))
            assert r.status_code == 200, r.text
            out = r.json()
    ...
    async def test_day_marks_derive_the_count_and_respect_the_window(self, seeded):
        ...
        async with await _client(seeded["app"]) as c:
            ...
            # The sent list REPLACED the 10 auto "no loads" marks, so
            # the excused lead/tail days now COUNT: 24 active ->
            # target 27,428.57, not met -> the rpm-only 1% tier.
            assert out["adjusted_target"] == 27_428.57
            assert out["pct"] == 1.0
