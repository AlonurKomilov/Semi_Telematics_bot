...
# 429 retry behaviour configured at module load.  Read directly from
# os.getenv (not via ``infra.config``) to avoid a circular import —
# ``infra/__init__.py`` imports the Samsara client, so the client
# can't reach back into the ``infra`` package at import time.  Tests
# monkeypatch these module-level names to verify the retry loop
# honours the configured limits.
...
def _parse_retry_after(value: str | None, default_sec: float) -> float:
    ...
    The header is supposed to be an integer count of seconds.  We
    accept floats too (lenient parse) and clamp pathological values
    so a hostile / buggy upstream can't make us sleep for hours.
    """
    if not value:
        return default_sec
    try:
        ...
        return default_sec
    # Clamp: at least 0.5 s (no busy-spin even if the upstream says 0),
    # at most 60 s (single retry never blocks an event loop for a
    # full minute regardless of what the upstream claims).
    return max(0.5, min(parsed, 60.0))


async def _sleep_with_jitter(seconds: float) -> None:
    """Sleep for ``seconds`` plus up to 10 % random jitter.

    The jitter prevents many concurrent callers from waking
    simultaneously and re-hammering the upstream — the "thundering
    herd" pattern that retried 429s would otherwise produce.
    """
    import asyncio
    import random
    jittered = seconds * (1.0 + random.random() * 0.1)
...
class SamsaraClient:
    ...
    async def get_org_id(self, *, force: bool = False) -> str | None:
        """Fetch and cache the Samsara organization ID via /me endpoint.

        ``force=True`` bypasses the cache so the connection probe
        (``SamsaraProvider.test_connection``) actually hits the
        network on every call.  Without this, a token that was revoked
        upstream after the first /me call would still report "ok"
        until the process restarted — the whole point of the probe
        is defeated by reading a stale cached value.
        """
        if not force and self._org_id is not None:
            return self._org_id
    ...
    async def _get_raw(self, endpoint: str, params: Optional[dict] = None) -> dict:
        ...
        # Read the retry constants at call time so monkeypatched
        # values in tests take effect without reloading the module.
        max_retries = SAMSARA_429_MAX_RETRIES
        default_retry_after = SAMSARA_429_DEFAULT_RETRY_AFTER_SEC
        ...
        # 429 retry loop — most calls exit on the first iteration.
        # Samsara responds 429 with a ``Retry-After`` header in
        # seconds when an endpoint's rate limit is exhausted.  We
        # honour it with a small jitter (+10 %) so concurrent
        ...
        while True:
            try:
                async with session.get(url, params=params) as resp:
                    if resp.status == 429:
                        if attempt >= max_retries:
                            status_label = "429"
                            logger.warning(
                                "Samsara 429 on %s — retries exhausted (%d)",
                                endpoint, max_retries,
                            )
                            resp.raise_for_status()  # raises ClientResponseError
                        # Read Retry-After (seconds) or fall back to a
                        # default — Samsara always sends it but a buggy
                        ...
                        status_label = "429"
                        logger.info(
                            "Samsara 429 on %s — sleeping %.1fs (attempt %d/%d)",
                            endpoint, retry_after, attempt, max_retries,
                        )
                        # Sleep then loop back into the same try
                        # block.  ``raise_for_status`` is not called
                        # on the 429 response — we discard it.
                        await _sleep_with_jitter(retry_after)
                        continue
                ...
                # Record on every iteration so 429-retry sequences
                # show up as separate observations rather than one
                # multi-second outlier.
                _obs.record_samsara(
                    endpoint=endpoint,
                    status=status_label,
    ...
    async def get_vehicles_overview(self) -> list[dict]:
        ...
        for vid, v in vehicles.items():
            ...
            entry = {
                ...
                "name": name,
                "vin": v.get("vin", "N/A"),
                "gateway_serial": ((v.get("gateway") or {}).get("serial") or ""),
                "make": v.get("make", "N/A"),
...
def build_multi_company_client(
    ...
    *,
    creds_map: dict[str, str] | None = None,
    fallback_token: str = "",
) -> "MultiCompanyClient":
    """Build a MultiCompanyClient from database Company objects.

    Token resolution order per company:
      1. ``creds_map[co.code]`` — the canonical source after the
         migration moves keys onto the Integration card.
      2. ``fallback_token`` — the legacy single-token slot on
         ``account_integrations.credentials.api_token``.
      3. ``co.samsara_api_key`` — the dual-source fallback that
         keeps existing read sites (camera/media/ai_maintenance/
         formatting/admin) working until they're refactored to go
    ...
    Args:
        ...
        creds_map: per-company key map loaded from the Integration
            credentials blob (``credentials["companies"]``).
        fallback_token: single legacy token from
            ``credentials["api_token"]`` — used when both the map and
            the company column are empty.
    Returns:
        A ready-to-use MultiCompanyClient.  Companies for which no
        token can be resolved are silently skipped — the dashboard's
        per-company key affordance prompts the operator to add one.
    """
    clients: dict[str, SamsaraClient] = {}
    creds_map = creds_map or {}
    skipped_codes: list[str] = []
    for co in companies:
        token = creds_map.get(co.code) or fallback_token or co.samsara_api_key
        if not token:
            # Operator hasn't added a key for this company yet — skip
            # rather than build a client with an empty token, which
            # would 401 on every request and burn quota.  Logged + the
            # dashboard's GET /integrations/samsara/companies endpoint
            # surfaces these as ``has_key=False`` so the operator can
            # set the key on the Integration card.
            skipped_codes.append(co.code)
            continue
    ...
    if skipped_codes:
        logger.warning(
            "build_multi_company_client acct=%s skipped %d compan%s "
            "with no API key: %s — set keys on Integration card",
            account_id, len(skipped_codes),
            "y" if len(skipped_codes) == 1 else "ies",
            ",".join(skipped_codes),
        )
