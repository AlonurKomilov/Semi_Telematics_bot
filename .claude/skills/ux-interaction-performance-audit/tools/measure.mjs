 ...
 * instrument of the ux-interaction-performance-audit skill.
 *
...
async function settle(page, t0) {
  ...
  return page.evaluate((t) => {
    ...
    return {
      settle: last ? Math.round(last.t + last.d - t) : 0,
...
for (const pageDef of profile.pages) {
  ...
  for (const throttle of THROTTLES) {
    ...
    for (const [name, runs] of Object.entries(gestures)) {
      ...
      console.log(`GESTURE ${name.padEnd(14)} INP: ${fmt(ok.map((r) => r.inp))} ms · settle: ${fmt(ok.map((r) => r.settle))} ms · worst task: ${fmt(ok.map((r) => r.worst))} ms`);
    }
  }
}
await browser.close();
